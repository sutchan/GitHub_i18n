...
function checkApiCompatibility() {
    ...
    const compatibility = {
        ...
        advancedFeatures: versionParts[0] >= 1 && versionParts[1] >= 5,  // 高级功能需要1.5.0+
        customModes: versionParts[0] >= 1 && versionParts[1] >= 6,  // 自定义模式需要1.6.0+
        eventSystem: versionParts[0] >= 1 && versionParts[1] >= 7  // 事件系统需要1.7.0+
    };
...
function safeApiCall(apiFunction, ...args) {
    ...
    if (!isCompatible) {
        console.error(`当前插件版本 ${version} 不兼容，请升级到 1.0.0 或更高版本`);
...
| 版本 | 发布日期 | 主要变更 |
|------|----------|----------|
| 1.0.0 | 2023-01-01 | 初始版本，基础翻译功能 |
| 1.1.0 | 2023-02-15 | 添加新的翻译词典，支持更多GitHub页面元素 |
| 1.2.0 | 2023-03-20 | 优化页面监控性能，减少资源占用 |
| 1.3.0 | 2023-04-10 | 修复已知问题，改进翻译质量，增加技术术语库 |
| 1.4.0 | 2023-05-05 | 增强版本检查功能，支持自动更新通知 |
| 1.5.0 | 2023-06-20 | 添加新的页面模式支持，优化工作流页面翻译 |
| 1.6.0 | 2023-07-30 | 优化缓存策略，提升重复访问时的加载速度 |
| 1.7.0 | 2023-09-10 | 改进错误处理机制，增强稳定性 |
| 1.8.0 | 2023-10-25 | 支持自定义翻译逻辑，增加API扩展性 |
| 1.8.88 | 2023-12-15 | 修复稳定性问题，优化性能，增强安全性 |

### API版本兼容性矩阵

| API模块 | 最低支持版本 | 向后兼容版本 | 推荐使用版本 |
|---------|------------|------------|------------|
| translationCore | 1.0.0 | 1.x.x | 1.8.0+ |
| pageMonitor | 1.0.0 | 1.x.x | 1.5.0+ |
| versionChecker | 1.4.0 | 1.x.x | 1.8.0+ |
| utils | 1.0.0 | 1.x.x | 1.7.0+ |
| 事件系统 | 1.7.0 | 1.x.x | 1.8.0+ |
| 自定义模式 | 1.6.0 | 1.x.x | 1.8.0+ |
...
|---------|---------|---------|--------|
| `initDictionary()` | 1.7.0 | 2.0.0 | 使用自动初始化或`GitHub_i18n.init()` |
| `translateElement(element)` | 1.8.0 | 2.0.0 | 使用`GitHub_i18n.translate(element)` |
| `detectPageMode()` | 1.8.0 | 2.0.0 | 使用`GitHub_i18n.getCurrentPageMode()` |
