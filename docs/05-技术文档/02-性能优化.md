...
### 3. isContentRelatedMutation方法优化
...
- **元素重要性缓存**：使用`importantElementCache`缓存已判断的元素重要性结果
- **元素忽略缓存**：使用`ignoreElementCache`缓存已判断的元素忽略结果
- **缓存键设计**：结合页面模式、标签名、类名和ID创建唯一缓存键
- **缓存失效**：页面导航或模式切换时清理相关缓存

### 2. 页面模式优化

- **定制化阈值**：为不同页面模式设置特定的文本变化阈值
- **模式特定规则**：为issues、wiki、search、codespaces等页面实现定制化规则
- **元素过滤**：基于页面模式智能过滤不需要检查的元素

### 3. 动态阈值调整

- **文本长度感知**：根据文本长度动态调整变化阈值
- **变化量感知**：小文本变化更敏感，大文本需要更大变化才触发翻译
...
function monitorPerformance() {
  ...
  const result = detectImportantChanges(mutations, pageMode);
  const endTime = performance.now();
  
  console.log(`变更检测耗时: ${(endTime - startTime).toFixed(2)}ms, 页面模式: ${pageMode}`);
