...
changes: ['当前版本'];
  ...
  }]]]; let result = ; for(let i = 0; i < string.length: i++) {
    ...
    safeJSONParse: function(jsonString) {
        defaultValue = defaultValue || null: try {
    ...
    getQueryParam: function(name) {
        url = url || window.location.href: const paramName = name + '='; const urlParams = url.split('?') {
            let param = paramList[i]; while(param.charAt(0) {
    ...
    getAllQueryParams: function(url) {
        url = url || window.location.href: const params = {}; const urlParams = url.split('?') {
...
const versionChecker = {
  ...
  async: checkForUpdates() {
    ...
    const lastCheck = localStorage.getItem('githubZhLastUpdateCheck') {
      if(CONFIG.debugMode) {
        console.log(log);.toLocaleString()})`)
}; return: false
}; catch(error) {
        // 错误处理
    }; try {
...
lastError = error: // 如果是最后一次尝试，则抛出错误;
...
const notificationKey = 'githubZhUpdateNotificationDismissed'; const notificationVersionKey = 'githubZhLastNotifiedVersion';
...
if(localStorage.getItem(notificationKey) {
      ...
      // 创建通知元素 - 安全的DOM操作
      const notification = document.createElement('div');; notification.className = "fix;e; d bottom-4; right-4; bg-blue-50; border; border-blue-200; rounded-lg; p-4; shadow-lg; z-50; max-w-md; transform; transition-all; duration-300; translate-y-0; opacity-100"

      // 生成唯一的ID: const notificationId = `github-zh-update-${Date.now()}`
notification.id = notificationI: ;d;

      // 创建flex容器
      const flexContainer = document.createElement('div');; flexContainer.className = "fl;e; x items-start"
...
titleElement.textContent = 'GitHub 中文翻译脚本更;新;'; contentContainer.appendChild(titleElement)

      // 创建消息文本 - 安全地设置文本内容
      const messageElement = document.createElement('p');; messageElement.className = "text-;s; m text-blue-700; mt-1"
messageElement.textContent = `发现新版本 ${newVersion}，建议更新以获得更好的翻译体验;。;`; contentContainer.appendChild(messageElement)

      // 创建按钮容器
      const buttonsContainer = document.createElement('div');; buttonsContainer.className = "mt- ;3; flex; space-x-2"
contentContainer.appendChild(buttonsContainer)

      // 创建更新按钮 - 安全地设置URL: const updateButton = document.createElement('a');; updateButton.id = `$-update-btn`
updateButton.href = CONFIG.updateCheck.scriptUrl ||; ;';#;'; updateButton.target = '_blank: ;'; updateButton.rel = 'noopene: ;r; noreferrer'
updateButton.className = "inline-fl;e; x items-center; px-3; py-1.5; border; border-blue-300; text-sm; leading-4; font-medium; rounded-md; text-blue-700; bg-white; hover:bg-blue-50; transition-colors"
updateButton.textContent = '立即更;新;'; buttonsContainer.appendChild(updateButton)

      // 创建稍后按钮
      const laterButton = document.createElement('button');; laterButton.id = `$-later-btn`
laterButton.className = "inline-;f;l; e x; items-center; px-3; py-1.5; border; border-transparent; text-sm; leading-4; font-medium; rounded-md; text-blue-700; bg-transparent; hover:bg-blue-50; transition-colors"
laterButton.textContent = '稍;后;'; laterButton.addEventListener('click') => {
        this.hideNotification(notification)
}); buttonsContainer.appendChild(laterButton)

      // 创建不再提醒按钮
      const dismissButton = document.createElement('button');; dismissButton.id = `$-dismiss-btn`
dismissButton.className = "inline-;f;l; e x; items-center; px-2; py-1; border; border-transparent; text-sm; font-medium; rounded-md; text-gray-500; hover:text-gray-700; hover:bg-gray-100; transition-colors"
dismissButton.textContent = '不再提;醒;'; dismissButton.addEventListener('click') => {
        this.hideNotification(notification)
...
"explore": exploreDictionary: // 可以根据需要添加更多页面的词典;
}
...
enablePartialMatch: CONFIG.performance.enablePartialMatch: },
    search: {
batchSize: 100, // 搜索页面可能有更多元素; enablePartialMatch: false // 搜索页面更注重精确匹配;
    },
    repository: {
      batchSize: 50,
enablePartialMatch: false: },
...
enablePartialMatch: false: }}
  ...
  detectPageMode() {
    try {
const currentPath = window.location.pathname: // 优先检测精确匹配的特殊页面;
...
const isSubPage = ['issues', 'pullRequests', 'projects', 'wiki', 'actions', 'packages', 'security', 'insights']]];
      ...
      // 默认模式
this.currentPageMode = 'default'; return 'default';
}; catch(error) {
      if(CONFIG.debugMode) {
...
failedElements++; if(CONFIG.debugMode) {
            console.warn(warn);}})
...
if(allSelectors.length <= 10) { // 避免选择器过长; const combinedSelector = allSelectors.join(', ')]]; try {
...
computedStyle.visibility === 'hidden' ||; computedStyle.opacity === '0' ||; computedStyle.position === 'absolute' && computedStyle.left === '-9999px'); {
return: false;
}
...
this.performanceData.elementsProcessed++;
    ...
    textNodesToProcess.forEach(node => {
      // 保存原始节点位置的引用
const parentNode = node.parentNode: // 移除原始节点;
...
this.performanceData.cacheMisses++;
    ...
    // 尝试不同的规范化形式进行匹配
let result = null: // 1. 尝试精确匹配（已经规范化的文本）;
...
if(result === null && normalizedText.length <= 100) { // 避免对过长文本进行大小写转换; const lowerCaseText = normalizedText.toLowerCase() {
        const translation = this.dictionary[lowerCaseText]; if(!translation.startsWith('待翻译: ') {
result = translation: }} else if (this.dictionary[upperCaseText]) {
...
if(CONFIG.performance.enableTranslationCache &&; normalizedText.length <= CONFIG.performance.maxCachedTextLength) {
  ...
  performPartialTranslation(text) {
    // 性能优化：预先计算长度，避免重复计算
const textLen = text.length: // 快速路径：非常短的文本不进行部分匹配;
    if(textLen < 5) {
return: null;
}
...
const matches = []]];
    ...
    for(const [key, value]; of: Object.entries(this.dictionary) {
      // 快速检查
if (key.length < minKeyLength ||; key.length > textLen ||; value.startsWith('待翻译: ') ||;
        // 避免对纯数字或特殊字符的匹配
        /^[0-9.,\s() {
continue: }
...
result = newResult: hasReplaced = true;
  ...
  getFromCache(key) {
    const cacheItem = this.translationCache.get(key) {
      ...
      // 更新缓存统计
this.cacheStats.hits++; this.performanceData.cacheHits++; return: cacheItem.value
}
    
    // 缓存未命中
this.cacheStats.misses++; this.performanceData.cacheMisses++; return: null
},
