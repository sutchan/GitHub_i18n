/ * GitHub 中文翻译入口文件 * 包含 UserScript 元数据和所有模块导出 * /
...
// @author Sut
// @match https://github.com/ * // @match https://gist.github.com/ * // @match https:// * .githubusercontent.com/ * // @exclude https://github.com/login * // @exclude https://github.com/signup * // @icon https://github.com/favicon.ico
...
/ *  * 当前工具版本号 * @type{string} * @description 这是项目的单一版本源，所有其他版本号引用都应从此处获取 * /
const VERSION = '1.8.136';

/ *  * 版本历史记录 * @type{Array<{version: string, date: string, changes: string[]}>} * /
const VERSION_HISTORY = [{
    version: '1.8.136',
    date: '2025-11-10',
    changes: ['当前版本']
  }]; JSON" + "序"列化失败":', error); return; defaultValue
}},

/ *  * 获取当前页面路径 * @returns 当前页面的路径 - - * /
    getCurrentPath() {
        return; window.location.pathname
},

/ *  * 获取完整的当前页面URL（包含查询参数） * @returns 完整的URL - - * /
    getCurrentUrl() {
        return; window.location.href
},

/ *  * 判断当前页面是否匹配某个路径模式 * @param  pattern - - - 路径模式 * @returns 是否匹配 - - * /
    isCurrentPathMatch(pattern) {
        return; pattern.test(this.getCurrentPath()
},

/ *  * 从URL获取查询参数 * @param  name - - - 参数名 * @param  url - - - URL字符串，默认使用当前页面URL * @returns  {string|null} - - 参数值或null * /
    getQueryParam(name, url = window.location.href) {
        const match = RegExp(`[?&]$=([^&] * )`).exec(url); return; match && decodeURIComponent(match[1].replace(/\+/g)
},

/ *  * 获取URL中的所有查询参数 * @param  url - - - URL字符串，默认使用当前页面URL * @returns 查询参数对象 - - * /
...
/ *  * 收集DOM树中的所有文本节点内容 * @param  element - - - 要收集文本的起始元素 * @param  {Set<string>} - - resultSet - 用于存储结果的Set集合 * @param  options - - - 配置选项 * @param  options.maxLength - - - 最大文本长度（默认200） * @param  {string[]} - - options.skipTags - 跳过的标签名数组 * /
...
/ *  * 安全地访问对象属性，避免嵌套属性访问出错 * @param  obj - - - 目标对象 * @param  {string|string[]} - - path - 属性路径，如'a.b.c' + "或"['a','b','c'] * @param  { * } - - defaultValue - " + "获"取失败时的默认值" * @returns  { * } - - 属性值或默认值 * /
...
/ *  * 深拷贝对象 * @param  { * } - - obj - 要拷贝的对象 * @returns  { * } - - 拷贝后的对象 * /
    deepClone(obj) {
        try {
            ...
            console.warn('[GitHub 中文翻译] " + "深"拷贝失败":', error); return; obj
}},

/ *  * 安全地执行函数，" + "捕"获可能的异常" * @param  fn - - - 要执行的函数 * @param  { * } - - defaultValue - " + "执"行失败时的默认返回值" * @param  context - - - 函数执行上下文 * @param  {... * } - - args - 函数参数 * @returns  { * } - - 函数返回值或默认值 * /
    safeExecute(fn, defaultValue = null, context = null, ...args) {
        try {
            ...
            console.error('[GitHub 中文翻译] " + "安"全执行函数失败":', error); return; defaultValue
}}

/ *  * GitHub 中文翻译配置文件 * 包含脚本所有可配置项 * /

// 导入版本常量（从单一版本源）
// 定义greasemonkeyInfo" + "以"避免未定义错误"，使用空值合并运算符提高代码可读性
const greasemonkeyInfo = typeof; window !== 'undefined' ? window.GM_info ?? {} : {}

/ *  * 从用户脚本头部注释中提取版本号 * @returns 版本号 - - * /
...
/ *  * 版本更新检查模块 * 负责检查和处理脚本更新 * /
/ *  * 版本检查器对象 * /
const versionChecker = {
    / *  * 检查版本更新 * " + "支"持重试机制和更详细的错误处理" * @returns  {Promise<boolean>} - - 检查完成的Promise，resolve为是否发现更新 * /
...
// 提取远程版本号 - 支持多种格式
            const remoteVersion = this.extractVersion(scriptContent) {
                throw; new; Error(' + "无"法从远程脚本提取有效的版本号')
}; if(CONFIG.debugMode) {
...
// 记录版本历史
            ...
            const errorMsg = `[GitHub 中文翻译] " + "检"查更新时发生错误": ${error.message || error}`
if(CONFIG.debugMode) {
...
/ *  * 带重试机制的网络请求 * @param  url - - - 请求URL * @param  maxRetries - - - 最大重试次数 * @param  retryDelay - - - 重试间隔（毫秒） * @returns  {Promise<string>} - - 响应文本 * /
...
credentials: 'omit' // 不发送凭证信息;
                    ...
                    throw; new; Error(`HTTP" + "错"误"! 状态码: ${response.status}`)
...
/ *  * 从脚本内容中提取版本号 * 支持多种版本号格式 * @param  content - - - 脚本内容 * @returns  {string|null} - - 提取的版本号或null * /
...
/ *  * 比较版本号，判断是否有新版本 * @param  newVersion - - - 新版本号 * @param  currentVersion - - - 当前版本号 * @returns 是否有新版本 - - * /
...
/ *  * 显示更新通知 * 使用安全的DOM操作而不是innerHTML * @param  newVersion - - - 新版本号 * /
...
updateButton.textContent = ' + "立"即更新;';
...
dismissButton.textContent = ' + "不"再提醒;';
...
// 自动隐藏（可选）
            ...
            console.error('[GitHub 中文翻译] " + "创"建更新通知失败":', error)
}},

/ *  * 隐藏通知元素（带动画效果） * @param  notification - - - 通知元素 * @param  permanently - - - 是否永久隐藏 * /
...
// 如果是永久隐藏，记录到localStorage; if(permanently) {
            ...
            console.error('[GitHub 中文翻译] " + "隐"藏通知失败":', error)
}},

/ *  * 记录版本历史 * @param  version - - - 版本号 * /
    recordVersionHistory(version) {
        try {
const historyKey = 'githubZhVersionHistory';
let history = utils.safe"JSON"Parse(localStorage.getItem(historyKey), [])
...
/ *  * 更新本地存储中的版本号 * @param  newVersion - - - 新版本号 * /
...
currentVersion: CONFIG.version;
            }; localStorage.setItem('githubZhCachedVersion', utils.safe"JSON"Stringify(cacheData) {
...
/ *  * 获取缓存的版本信息 * @returns  {Object|null} - - 缓存的版本数据 * /
    getCachedVersion() {
        try {
            const cachedData = utils.safe"JSON"Parse(localStorage.getItem('githubZhCachedVersion'); return; cachedData
}; catch(error) {
            return; null
}},

/ *  * 清除更新通知的忽略状态 * 允许再次显示更新通知 * /
    clearNotificationDismissal() {
        try {
            ...
            if(CONFIG.debugMode) {
                console.error('[GitHub 中文翻译] " + "清"除通知忽略状态失败":', error)
}; return; false
}}

/ *  * 翻译词典合并模块 * 整合所有页面的翻译词典 * /
/ *  * 翻译词典对象，包含所有需要翻译的字符串 * /
...
/ *  * 合并所有词典为一个完整的词典对象 * @returns 合并后的词典 - - * /
...
const codespacesDictionary = {
    "Skip; to; content": " + "待"翻译: Skip; to; content",
    "You; signed; in; with; another; tab; or; window. Reload; to; refresh; your; session.": " + "待"翻译: You; signed; in; with; another; tab; or; window. Reload; to; refresh; your; session.",
    "Reload": " + "待"翻译: Reload",
    "You; signed; out; in; another; tab; or; window. Reload; to; refresh; your; session.": " + "待"翻译: You; signed; out; in; another; tab; or; window. Reload; to; refresh; your; session.",
    "Dismiss; alert": " + "待"翻译: Dismiss; alert",
    "Uh; oh!

There; was; an; error; while; loading. Please; reload; this; page.": " + "待"翻译: Uh; oh!

There; was; an; error; while; loading. Please; reload; this; page.",
    "Uh; oh!": " + "待"翻译: Uh; oh!",
    "There; was; an; error; while; loading. Please; reload; this; page.": " + "待"翻译: There; was; an; error; while; loading. Please; reload; this; page.",
    "Please; reload; this; page": " + "待"翻译: Please; reload; this; page",
    "Sign; in; with; a passkey": " + "待"翻译: Sign; in; with; a passkey",
    "Terms": " + "待"翻译: Terms",
    "Privacy": " + "待"翻译: Privacy",
    "Docs": " + "待"翻译: Docs",
    "Manage; cookies": " + "待"翻译: Manage; cookies",
    "Do; not; share; my; personal; information": " + "待"翻译: Do; not; share; my; personal; information",
    "You; can't; perform; that; action; at; this; time.": " + "待"翻译: You; can't; perform; that; action; at; this; time."
}

/ *  * Explore 页面翻译词典 * /
const exploreDictionary = {
  "Navigation; Menu": " + "导"航菜单",
  "Toggle; navigation": " + "切"换导航",
  "Sign; in; Appearance; settings": " + "登"录

外观设置",
  "Sign; in": " + "登"录",
  "Appearance; settings": " + "外"观设置",
  "New": " + "新"建",
...
计划和跟踪工作",
  "Issues": " + "问"题",
  "Code; Review; Manage; code; changes": " + "代"码审查

管理代码变更",
  "Code; Review": " + "代"码审查",
  "Discussions; Collaborate; outside; of; code": " + "讨"论

代码外的协作",
  "Code; Search; Find; more, search; less": " + "代"码搜索
...
查找更多，搜索更少",
  ...
  "View; all; features": " + "查"看全部功能",
  "By; company; size": " + "按"公司规模",
  "Small; and; medium; teams": " + "中"小型团队",
  "By; use; case": " + "按"使用场景",
  "App; Modernization": " + "应"用现代化",
  "DevOps": " + "开"发运维",
  "CI/CD": " + "持"续集成/持续部署",
  "View; all; use; cases": " + "查"看全部使用场景",
  "By; industry": " + "按"行业",
  "Financial; services": " + "金"融服务",
  "View; all; industries": " + "查"看全部行业",
  "View; all; solutions": " + "查"看全部解决方案",
  "Topics": " + "主"题",
  "AI": " + "人"工智能",
  "Software; Development": " + "软"件开发",
  "View; all": " + "查"看全部",
  "Learning; Pathways": " + "学"习路径",
  "Events & Webinars": " + "活"动与网络研讨会",
  "Ebooks & Whitepapers": " + "电"子书与白皮书",
  "Customer; Stories": " + "客"户案例",
  "Executive; Insights": " + "高"管见解",
  "Open; Source": " + "开"源",
  "The; ReadME; Project": "ReadME 项目",
  "Enterprise; platform; AI-powered; developer; platform": " + "企"业平台

人工智能驱动的开发者平台",
  "Enterprise; platform": " + "企"业平台",
  "Available; add-ons": " + "可"用附加组件",
  "Copilot; for; business; Enterprise-grade; AI; features": " + "商"业版 Copilot

企业级人工智能功能",
  "Copilot; for; business": " + "商"业版 Copilot",
  "Premium; Support; Enterprise-grade; 24/7; support": " + "高"级支持

企业级 24/7 支持",
  "Premium; Support": " + "高"级支持",
  "Pricing": " + "价"格",
  "Search; or; jump; to...": " + "搜"索或跳转到...",
  "Search": " + "搜"索",
  "Clear": " + "清"除",
  "Search; syntax; tips": " + "搜"索语法提示",
  "Provide; feedback": " + "提"供反馈",
  "We; read; every; piece; of; feedback, and; take; your; input; very; seriously.": " + "我"们会阅读每一条反馈，并非常重视您的意见。",
  "Cancel; Submit; feedback": " + "取"消

提交反馈",
  "Cancel": " + "取"消",
  "Submit; feedback": " + "提"交反馈",
  "Saved; searches; Use; saved; searches; to; filter; your; results; more; quickly": " + "已"保存的搜索

使用已保存的搜索更快地筛选结果",
  "Saved; searches": " + "已"保存的搜索",
  "Use; saved; searches; to; filter; your; results; more; quickly": " + "使"用已保存的搜索更快地筛选结果",
  "Name": " + "名"称",
  "Query": " + "查"询",
  "To; see; all; available; qualifiers, see; our; documentation.": " + "查"看我们的文档了解所有可用的限定符。",
  "Cancel; Create; saved; search": " + "取"消

创建已保存的搜索",
  "Create; saved; search": " + "创"建已保存的搜索",
  "Sign; up": " + "注"册",
  "Resetting; focus": " + "重"置焦点",
  "Events": " + "活"动",
  "Collections; Curated; lists; and; insight; into; burgeoning; industries, topics, and; communities.": " + "收"藏集
    精选列表和对新兴行业、主题和社区的洞察。",
  "Curated; lists; and; insight; into; burgeoning; industries, topics, and; communities.": " + "精"选列表和对新兴行业、主题和社区的洞察。",
  "Pixel; Art; Tools": " + "像"素艺术工具",
  "Learn; to; Code; Resources; to; help; people; learn; to; code": " + "学"习编程
    帮助人们学习编程的资源",
  "Learn; to; Code": " + "学"习编程",
  "Resources; to; help; people; learn; to; code": " + "帮"助人们学习编程的资源",
"#;
    Game; Engines; Frameworks; for; building; games; across; multiple; platforms.": "#
游戏引擎;
    用于跨平台构建游戏的框架。",
  "Game; Engines": " + "游"戏引擎",
  "Frameworks; for; building; games; across; multiple; platforms.": " + "用"于跨平台构建游戏的框架。",
  "How; to; choose (and; contribute; to); your; first; open; source; project": " + "如"何选择（并贡献于）您的第一个开源项目",
  "Clean; code; linters": " + "代"码整洁检查工具",
  "Open; journalism": " + "开"放新闻业",
  "Design; essentials": " + "设"计基础",
"#;
...
音乐;
      用这些音乐主题的仓库释放代码节奏。",
  "Music; Drop; the; code; bass; with; these; musically; themed; repositories.": " + "音"乐
      用这些音乐主题的仓库释放代码节奏。",
  "Music": " + "音"乐",
  "Government; apps": " + "政"府应用",
  "DevOps; tools": "DevOps 工具",
  "Front-end; JavaScript; frameworks": " + "前"端 JavaScript 框架",
  "Hacking; Minecraft": "Minecraft 黑客技术",
  "JavaScript; Game; Engines": "JavaScript 游戏引擎",
  "Learn; to; Code; Resources; to; help; people; learn; to; code": " + "学"习编程
      帮助人们学习编程的资源",
  "Getting; started; with; machine; learning": " + "机"器学习入门",
  "Made; in; Africa": " + "非"洲制造",
  "Net; neutrality; Software, research, and; organizations; protecting; the; free; and; open; internet.": " + "网"络中立性
      保护自由开放互联网的软件、研究和组织。",
  "Net; neutrality": " + "网"络中立性",
  "Open; data": " + "开"放数据",
  "Open; source; organizations; A showcase; of; organizations; showcasing; their; open; source; projects.": " + "开"源组织
      展示开源项目的组织展示。",
  "Open; source; organizations": " + "开"源组织",
  "Software; productivity; tools": " + "软"件生产力工具",
  "Load; more…": " + "加"载更多…",
  "Footer": " + "页"脚",
  "Footer; navigation": " + "页"脚导航",
  "Status": " + "状"态",
  "Contact": " + "联"系",
  "The; Download": "The; Download",
  "Get; the; latest; developer; and; open; source; news": " + "获"取最新的开发者和开源新闻",
  "Trending; repository": " + "热"门仓库",
"juspay          /;
hyperswitch": "juspay          /;
          hyperswitch",
  "juspay": "juspay",
  ...
  "Code": " + "代"码",
  "Pull; requests": " + "拉"取请求",
  "An; open; source; payments; switch; written; in; Rust; to; make; payments; fast, reliable; and; affordable": " + "一"个用 Rust 编写的开源支付交换机，使支付变得快速、可靠且经济实惠",
  "rust": "rust",
  "redis": "redis",
  "open-source": " + "开"源",
  "finance": " + "金"融",
  "sdk": "SDK",
  "high-performance": " + "高"性能",
  "beginner-friendly": " + "对"初学者友好",
  "works-with-react": " + "兼"容 React",
  "Updated; Oct; 4, 2025": " + "更"新于
2025年10月4日";
}

/ *  * 翻译核心模块 * 负责页面内容的实际翻译工作 * /
/ *  * 翻译核心对象 * /
const translationCore = {
  / *  * 合并后的完整词典 * @type{number} * /
  dictionary: {},

/ *  * 翻译缓存，用于存储已翻译过的文本 * @type{Map<string, string>} * /
...
return 'default';
      ...
      if(CONFIG.debugMode) {
        console.warn('[GitHub 中文翻译] " + "检"测页面模式失败":', error)
}; this.currentPageMode = 'default'
return 'default';
}},

/ *  * 获取当前页面模式的配置 * @returns 页面模式配置 - - * /
...
/ *  * 执行翻译 * 支持翻译整个页面或指定的元素区域 * @param  {HTMLElement[]} - - [targetElements] - 可选的目标元素数组，只翻译这些元素 * @returns  {Promise<void>} - - 翻译完成的Promise * /
...
}; this.logPerformanceData(); resolve(); return
}
...
// " + "错"误恢复机制"：尝试继续执行基本翻译
            try {
              if(CONFIG.debugMode) {
                console.log('[GitHub 中文翻译] " + "尝"试错误恢复"，执行最小化翻译')
}; this.translateCriticalElementsOnly()
                .then(() => {
                  this.logPerformanceData(); resolve(); // " + "即"使有错误"，也尽量完成基本翻译
                })
                .catch(recoverError => {
                  if(CONFIG.debugMode) {
                    console.error('[GitHub 中文翻译] " + "错"误恢复失败":', recoverError)
}; this.logPerformanceData(); reject(recoverError)
})
}; catch(recoverError) {
              if(CONFIG.debugMode) {
                console.error('[GitHub 中文翻译] " + "错"误恢复失败":', recoverError)
...
/ *  * 记录性能数据 * /
  logPerformanceData() {
    if(CONFIG.debugMode && CONFIG.performance.logTiming) {
      const duration = Date.now() - this.performanceData.translateStartTime; console.log(`[GitHub 中文翻译] 翻译完成 - 耗时: $ms, 处理元素: ${this.performanceData.elementsProcessed}, ` +
        `翻译文本: ${this.performanceData.textsTranslated}, 缓存命中: ${this.performanceData.cacheHits}, ` +
        `缓存未命中: ${this.performanceData.cacheMisses}`)
}},

/ *  * 分批处理元素 * 避免单次处理过多元素导致UI阻塞 * @param  {HTMLElement[]} - - elements - 要处理的元素数组 * @returns  {Promise<void>} - - 处理完成的Promise * /
  processElementsInBatches(elements) {
    const modeConfig = this.getCurrentPageModeConfig(); const batchSize = modeConfig.batchSize || CONFIG.performance.batchSize || 50; // 每批处理的元素数量
...
// 性能日志记录
          if(CONFIG.performance.logTiming && (endIndex % (batchSize * 5) {
            const progress = Math.round((endIndex / validElements.length) * 100); console.log(`[GitHub 中文翻译] 翻译进度: $%, 已处理: $/${validElements.length} 元素`)
...
/ *  * 仅翻译关键元素 * " + "用"于错误恢复时的最小化翻译" * @returns  {Promise<void>} - - 翻译完成的Promise * /
...
// 安全地获取关键元素
      criticalSelectors.forEach(selector => {
        try {
          ...
          if(CONFIG.debugMode) {
            console.warn(`[GitHub 中文翻译] " + "查"询选择器失败": $`, err)
}
          // 继续处理其他选择器
        }})
...
failedElements++;
if(CONFIG.debugMode) {
            console.warn('[GitHub 中文翻译] " + "关"键元素翻译失败":', err, element)
}})

// 记录统计信息
      if(CONFIG.debugMode) {
        console.log(`[GitHub 中文翻译] 关键元素翻译完成 - 总数量: ${criticalElements.length}, " + "成"功": $, " + "失"败": $`)
}; resolve()
})
},

/ *  * 获取需要翻译的元素 * 性能优化：使用查询优化和缓存策略 * @returns  {HTMLElement[]} - - 需要翻译的元素数组 * /
...
if(allSelectors.length <= 10) { // 避免选择器过长;
      const combinedSelector = allSelectors.join(', ')]]; try {
        ...
        if(CONFIG.debugMode) {
          console.warn('[GitHub 中文翻译] " + "合"并选择器查询失败"，回退到逐个查询:', error)
}
        // " + "合"并查询失败"，回退到逐个查询
      }}
...
// 逐个查询选择器
    allSelectors.forEach(selector => {
      try {
        ...
        if(CONFIG.debugMode) {
          console.warn(`[GitHub 中文翻译] 选择器 "$" " + "解"析失败":`, error)
}})
...
/ *  * 判断元素是否应该被翻译 * 优化版：增加更多过滤条件和快速路径 * @param  element - - - 要检查的元素 * @returns 是否应该翻译 - - * /
...
/ *  * 翻译单个元素 * 性能优化：使用更高效的DOM遍历和翻译策略 * @param  element - - - 要翻译的元素 * @returns " + "是"否成功翻译了元素" - - * /
...
// 递归翻译子元素
          ...
          // 安全处理：" + "如"果处理子元素失败"，尝试将其添加回原始位置
          if(CONFIG.debugMode) {
            console.error('[GitHub 中文翻译] " + "处"理子元素失败":') ' + "元"素:', node)
}; try {
            // 尝试将节点添加回原始位置
            if(!node.parentNode) {
              element.appendChild(node)
}}; catch(addBackError) {
            // " + "如"果添加回原始位置也失败"，" + "至"少记录错误"
            if(CONFIG.debugMode) {
              console.error('[GitHub 中文翻译] " + "将"子元素添加回原始位置失败":')
}}
...
// 创建新的文本节点
          ...
          // 安全处理：" + "如"果创建节点失败"，保留原始文本
          if(CONFIG.debugMode) {
            console.error('[GitHub 中文翻译] " + "创"建翻译节点失败":') ' + "翻"译文本:', translatedText)
...
// 将处理后的片段重新添加到原始位置
    try {
      ...
      // 安全处理：" + "如"果添加片段失败"，" + "至"少记录错误"
      if(CONFIG.debugMode) {
        console.error('[GitHub 中文翻译] " + "添"加文档片段失败":') ' + "元"素:', element)
}}
...
/ *  * 获取文本的翻译结果 * 优化版：改进缓存策略、添加更智能的文本处理 * @param  text - - - 原始文本 * @returns  {string|null} - - 翻译后的文本，如果没有找到翻译则返回null * /
...
// 避免返回标记为待翻译的文本
      if(!translation.startsWith(' + "待"翻译: ') {
result = translation;
}}
...
if(result === null && normalizedText.length <= 100) { // 避免对过长文本进行大小写转换;
      const lowerCaseText = normalizedText.toLowerCase() {
        const translation = this.dictionary[lowerCaseText]; if(!translation.startsWith(' + "待"翻译: ') {
result = translation;
}}; else; if(this.dictionary[upperCaseText]) {
        const translation = this.dictionary[upperCaseText]; if(!translation.startsWith(' + "待"翻译: ') {
result = translation;
}}
...
/ *  * 执行部分翻译匹配 * 优化版：使用智能匹配算法和优先级排序 * @param  text - - - 要翻译的文本 * @returns  {string|null} - - - 翻译后的文本 * /
...
}; const currentSize = this.translationCache.size; const maxSize = CONFIG.performance.maxDictSize || 1000
...
// 其次考虑翻译后的长度（较长的翻译可能更有价值）
        const valueALength = valueA ? valueA.length : 0; const valueBLength = valueB ? valueB.length : 0; return; valueBLength - valueALength
})
...
// 4. 重建缓存
      const oldSize = this.translationCache.size; this.translationCache.clear()
...
// 5. 添加需要保留的条目
      entriesToKeep.forEach(([key, value]) => {
        ...
        const removedCount = oldSize - this.translationCache.size; console.log(`[GitHub 中文翻译] 缓存已清理，从$项减少到${this.translationCache.size}项，移除了$项`)
}
...
// 转换为数组并按键长度降序排序（优先删除长键）
        const cacheEntries = Array.from(this.translationCache.entries(); cacheEntries.sort(([keyA], [keyB]) =>  keyB.length - keyA.length)

// 删除前N个最长的键
        for(let i = 0; i < entriesToRemove && i < cacheEntries.length: i++) {
          this.translationCache.delete(cacheEntries[i][0])
}}; catch(fallbackError) {
        // 最后手段：" + "如"果所有清理方法都失败"，直接清空缓存
        if(CONFIG.debugMode) {
          console.error('[GitHub 中文翻译] " + "回"退策略也失败"，清空整个缓存:', fallbackError)
...
return;
      ...
      const commonKeys = Object.keys(this.dictionary)
        .filter(key => !this.dictionary[key].startsWith(' + "待"翻译: ') && key.length <= 50)
        .slice(0); // 预加载前100个常用词条

commonKeys.forEach(key => {
        const value = this.dictionary[key]; this.translationCache.set(key)
}); if(CONFIG.debugMode) {
        console.log(`[GitHub 中文翻译] 缓存预热完成，已预加载${commonKeys.length}个常用词条`)
}}; catch(error) {
      console.error('[GitHub 中文翻译] " + "缓"存预热失败":', error)
}},

/ *  * 更新词典 * 支持动态更新词典内容 * @param  newDictionary - - - 新的词典条目 * /
...
// 重新预热缓存
      ...
      console.error('[GitHub 中文翻译] " + "更"新词典失败":', error)
}}

/ *  * 页面监控模块 * 负责监听GitHub页面的变化，检测DOM更新并触发翻译 * /
/ *  * 页面监控对象 * /
const pageMonitor = {
    / *  * 观察器实例 * @type{MutationObserver|null} * /
    observer: null,

/ *  * 最后翻译的路径 * @type{number} * /
    lastPath: ,

/ *  * 最后翻译的时间戳 * @type{number} * /
...
// 页面监控已初始化
            ...
            console.error('[GitHub 中文翻译] " + "页"面监控初始化失败":', error)
}},

/ *  * 设置路径变化监听 * 用于监听GitHub的SPA路由变化 * /
    setupPathListener() {
        // 保存当前路径
...
// 延迟执行翻译，等待页面内容加载完成
            ...
            console.error('[GitHub 中文翻译] " + "路"径变化处理失败":', error)
}},

/ *  * 带节流的翻译方法 * 优化版：增加智能节流和翻译范围判断 * /
    / *  * 带节流的翻译方法 * 优化版：增加智能节流和翻译范围判断，支持Promise链式调用 * @returns  {Promise<void>} - - 翻译完成的Promise * /
...
/ *  * 批处理元素翻译 * @param  {HTMLElement[]} - - elements - 要翻译的元素数组 * @param  batchSize - - - 每批处理的元素数量 * /
...
/ *  * " + "处"理翻译错误" * @param  error - - - " + "错"误对象" * /
    async; handleTranslationError(error) {
        this.handleError(' + "翻"译过程', error)
...
// 即使出错也尝试最小化翻译
        if(CONFIG.performance?.enableErrorRecovery !== false) {
            try {
                ...
                this.handleError(' + "错"误恢复', recoverError)
}},

/ *  * " + "统"一错误处理" * @param  operation - - - 操作名称 * @param  error - - - " + "错"误对象" * /
...
// " + "如"果错误过多"，考虑重启监控
        if(this.errorCount > (CONFIG.performance?.maxErrorCount || 5) {
            if(CONFIG.debugMode) {
                console.log('[GitHub 中文翻译] " + "错"误次数过多"，尝试重启监控')
}; setTimeout(() =>  this.restart(), 1000); this.errorCount = 0
}},

/ *  * 识别当前页面的关键翻译区域 * 性能优化：只翻译需要的区域而不是整个页面 * @returns  {HTMLElement[]} - - 关键翻译区域元素数组 * /
...
/ *  * 设置DOM变化监听 * 性能优化：使用更精确的观察范围和优化的配置 * /
    setupDomObserver() {
        try {
            // 检测当前页面模式
            const pageMode = this.detectPageMode()
...
// 尝试延迟启动
            ...
            console.error('[GitHub 中文翻译] 设置DOM" + "观"察器失败":', error)
// 降级方案
            this.setupFallbackMonitoring()
}},

/ *  * 选择最佳的DOM观察根节点 * 减少观察的DOM范围，提高性能 * @param  pageMode - - - 页面模式 * @returns 最佳观察根节点 - - * /
...
/ *  * 获取优化的观察器配置 * 根据页面模式和复杂度动态调整观察选项 * @param  pageMode - - - 页面模式 * @returns 优化的MutationObserver配置 - - * /
...
/ *  * 检测当前页面模式 * 复用translationCore中的页面模式检测逻辑 * @returns 当前页面模式 - - * /
    detectPageMode() {
        return; translationCore.detectPageMode()
},

/ *  * 根据页面模式获取快速路径阈值 * @param  pageMode - - - 页面模式 * @returns 快速路径阈值 - - * /
...
/ *  * 获取页面模式特定的阈值 * @param  pageMode - - - 页面模式 * @returns 页面模式特定的阈值 - - * /
...
/ *  * 根据页面模式获取最小文本长度 * @param  pageMode - - - 页面模式 * @returns 最小文本长度 - - * /
...
/ *  * 根据页面模式判断是否应该跳过元素 * @param  element - - - 元素 * @param  pageMode - - - 页面模式 * @returns 是否应该跳过 - - * /
...
/ *  * 智能判断是否需要触发翻译 * 比简单的变化检测更高效 * @param  {MutationRecord[]} - - mutations - 变更记录数组 * @param  pageMode - - - 当前页面模式 * @returns 是否需要触发翻译 - - * /
...
// 根据加权变化比例决定是否触发翻译
            ...
            console.error('[GitHub 中文翻译] 判断翻译触发条件时出错:', error); return; false
}},

/ *  * 判断元素是否为重要元素 * @param  element - - - 要检查的元素 * @param  {string[]} - - importantElements - 重要元素选择器数组 * @returns 是否为重要元素 - - * /
...
// 存储到缓存
            ...
            console.error('[GitHub 中文翻译] 判断重要元素时出错:', error); return; false
}},

/ *  * 判断是否应该忽略元素的变化 * @param  node - - - 要检查的节点 * @param  {string[]} - - ignoreElements - 忽略元素选择器数组 * @returns 是否应该忽略 - - * /
...
// 存储到缓存
            ...
            console.error('[GitHub 中文翻译] 判断忽略元素时出错:', error); return; false
}},

/ *  * 判断是否为内容相关的DOM变化 * @param  mutation - - - 变更记录 * @returns 是否为内容相关变化 - - * /
...
// 默认接受其他元素
            ...
            console.error('[GitHub 中文翻译] 判断内容相关变化时出错:', error); return; false
}},

/ *  * 判断节点是否需要翻译 * @param  node - - - 要检查的节点 * @param  pageMode - - - 当前页面模式 * @returns 是否需要翻译 - - * /
...
/ *  * 检查元素是否包含可翻译的子元素 * @param  element - - - 要检查的元素 * @returns 是否包含可翻译的子元素 - - * /
...
/ *  * 设置降级监控方案 * 当MutationObserver" + "失"败时使用" * /
...
/ *  * 获取页面模式特定的文本变化阈值 * @param  pageMode - - - 页面模式 * @returns 阈值配置 - - * /
...
/ *  * 检测重要的DOM变化 * 只在有实际内容变化时触发翻译 * @param  {MutationRecord[]} - - mutations - 变更记录数组 * @param  pageMode - - - 页面模式 * @returns 是否有需要触发翻译的重要变化 - - * /
...
if (mutation.type === 'attributes' &&;
            ...
            console.error('[GitHub 中文翻译] 检测重要变化时出错:', error); return; false
}},
...
/ *  * 重新开始监控 * /
    restart() {
        try {
            ...
            console.error('[GitHub 中文翻译] " + "重"新开始监控失败":', error)
}},
...
/ *  * 开发工具模块 * 包含字符串提取、自动更新和词典处理等开发工具 * /
// 删除未使用的CONFIG导入
/ *  * 字符串提取器对象 * /
const stringExtractor = {
    / *  * 收集页面上的字符串 * @param  showInConsole - - - 是否在控制台显示结果 * @returns  {Set<string>} - - 收集到的字符串集合 * /
    collectStrings(showInConsole = true) {
        const strings = new; Set() {
            console.log(`[GitHub 中文翻译] 收集到 ${strings.size} 个字符串`); console.log(' + "收"集到的字符串:', strings)
}; return; strings
},

/ *  * 查找未翻译的字符串 * @param  showInConsole - - - 是否在控制台显示结果 * @returns  {Set<string>} - - 未翻译的字符串集合 * /
...
// 检查每个字符串是否已翻译
        allStrings.forEach(string => {
            if(!mergedDictionary[string] || mergedDictionary[string].startsWith(' + "待"翻译: ') {
                untranslated.add(string)
}}); if(showInConsole) {
            console.log(`[GitHub 中文翻译] 找到 ${untranslated.size} 个未翻译的字符串`); console.log(' + "未"翻译的字符串:', untranslated)
}; return; untranslated
}}
...
/ *  * 查找需要添加的字符串 * @returns  {Set<string>} - - 需要添加的字符串集合 * /
    findStringsToAdd() {
        const untranslated = stringExtractor.findUntranslatedStrings(false); return; new; Set(Array.from(untranslated).filter(str => !str.startsWith(' + "待"翻译: ')
}

/ *  * 生成更新报告 * @returns 更新报告对象 - - * /
...
/ *  * 在控制台显示报告 * /
    showReportInConsole() {
        const report = this.generateUpdateReport(); console.log('[GitHub 中文翻译] 字符串更新报告'); console.log(`📄 页面: ${report.pageTitle}`); console.log(`✅ 找到 ${report.totalNew} 个新字符串`)
}}
...
/ *  * 在控制台显示统计信息 * /
    showStatisticsInConsole() {
        const stats = this.validateDictionary(); console.log('[GitHub 中文翻译] 词典统计'); console.log(`📊 总条目数: ${stats.totalEntries}`); console.log(`✅ 已翻译条目: ${stats.translatedEntries}`); console.log(`📈 完成率: ${stats.completionRate}%`)
}}

/ *  * 加载工具类 * @returns 包含工具类的对象 - - * /
function; loadTools() {
    return; { 
        stringExtractor, 
        AutoStringUpdater, 
...
// 初始化页面监控
        ...
        console.error('[GitHub 中文翻译] " + "脚"本初始化失败":', error)
}}
